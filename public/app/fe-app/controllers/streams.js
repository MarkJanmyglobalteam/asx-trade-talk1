...
asxTradeTalkApp.controller('streamsCtrl', function($scope, $uibModal, Config, Auth, moment, store, $window, $firebaseObject, $ngConfirm, Utils, Socialshare, $filter, $timeout, streamsFtry, mainFtry, $firebaseArray, FirebaseDatabase, FirebaseStorageRef, $rootScope, EODSvc, toastr){
        ...
        $scope.streams.$loaded().then(function(data){
              angular.forEach(data, function(stream, key){
                   ...
                   angular.forEach(stream, function(inStream, key){
                      if(inStream){
                          if(typeof inStream === 'object' && streamUID != uid){
